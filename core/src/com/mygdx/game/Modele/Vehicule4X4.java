...
public class Vehicule4X4 implements VehiculeDef{
    ...
    //réglages
    private float boostPower = 150;
    private Vector2 jumpPower = new Vector2(75, 75);
    ...
    private int wheelFixtureDefFriction = 250;
    private float wheelFixtureDefRestitution = 0.001f;
    private float deadBoxShapeRadius = 10;
    private Vector2 neckLocalAnchorAPosition = new Vector2(-5,13.2f);
    private Vector2 neckLocalAnchorBPosition = new Vector2(-2,-8.5f);
    private Vector2 wheelLocalAnchorAPosition;
    private float axisDefFrequencyHz = 3.5f;
    private float scaleSprite = 0.25f;
    private int prime = 125;
    private float tachyDeadTimer = 0;

    private int randomVehicule;
    private String accelSoundRef = "4X4SoundEffect-accel.mp3";
    private String decelSoundRef = "4X4SoundEffect-decel.mp3";
    private String atlasRef = "postLoading_Vehicule4X4.atlas";

    public Vehicule4X4(MTCGame mtcGame, String player) {
        this.player = player;
        ...
        wheelTexture = textureAtlas.findRegion("4x4_wheel");
        crashedWheelTexture = textureAtlas.findRegion("4x4_crashedWheel");

        if(player.equals("ok")){
            this.vehiculeSoundAccel = mtcGame.getAssetManager().get("4X4SoundEffect-accel.mp3", Music.class);
            this.vehiculeSoundDecel = mtcGame.getAssetManager().get("4X4SoundEffect-decel.mp3", Music.class);

            TextureAtlas textureAtlasTabBord = mtcGame.getAssetManager().get("postLoading_Vehicule4X4.atlas",TextureAtlas.class);
            jaugeCadran = new Image(textureAtlasTabBord.findRegion("4X4_jauge_cadran"));
            ...
            tableauDeBordTexture = new Image(textureAtlasTabBord.findRegion("tabBord-4X4"));
            tableauNITROTexture = new Image(textureAtlasTabBord.findRegion("tabBord-4X4-NITRO"));
            tachyCadran = new Image(textureAtlasTabBord.findRegion("4X4_tachy_cadran"));
        ...
        chassisShapeFloatArray = new float[]{-carWidth / 2, -carHeight / 1.6f, carWidth / 2.5f,
                -carHeight / 1.6f, carWidth / 2.5f, carHeight *0.7f, -carWidth / 2, carHeight *0.7f};
    ...
    private TextureRegion createChassisTexture(){
        if(player.equals("ok")){
            chassisTexture = textureAtlas.findRegion("4X4_chassis");
        }else{
            randomVehicule = MathUtils.random(0,3);
            if(randomVehicule == 0){
                chassisTexture = textureAtlas.findRegion("4X4_chassis-2");
            }
            if(randomVehicule == 1){
                chassisTexture = textureAtlas.findRegion("4X4_chassis-3");
            }
            if(randomVehicule == 2){
                chassisTexture = textureAtlas.findRegion("4X4_chassis-4");
            }
            if(randomVehicule == 3){
                chassisTexture = textureAtlas.findRegion("4X4_chassis-5");
            }
        }
    ...
    private TextureRegion createCrashedChassisTexture(){
        if(player.equals("ok")){
            crashedChassisTexture = textureAtlas.findRegion("4X4_crashedChassis");
        }else{
            if(randomVehicule == 0){
                crashedChassisTexture = textureAtlas.findRegion("4X4_crashedChassis-2");
            }
            if(randomVehicule == 1){
                crashedChassisTexture = textureAtlas.findRegion("4X4_crashedChassis-3");
            }
            if(randomVehicule == 2){
                crashedChassisTexture = textureAtlas.findRegion("4X4_crashedChassis-4");
            }
            if(randomVehicule == 3){
                crashedChassisTexture = textureAtlas.findRegion("4X4_crashedChassis-5");
            }
        }
    ...
    private Vector2 createWheelLocalAnchorAPosition(){
        if(player.equals("ok")){
            wheelLocalAnchorAPosition  = new Vector2(-18.33f,-12.8f);
        }else{
            wheelLocalAnchorAPosition  = new Vector2(-18.33f,-12.8f);
        }
