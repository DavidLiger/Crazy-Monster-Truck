...
public class VehiculeGT implements VehiculeDef {
    ...
    private Vector2 jumpPower = new Vector2(0, 90);
    ...
    private Vector2 neckLocalAnchorAPosition = new Vector2(-5,8.5f);
    private Vector2 neckLocalAnchorBPosition = new Vector2(-2,-8.5f);
    private Vector2 wheelLocalAnchorAPosition;
    private float axisDefFrequencyHz = 3.2f;
    private float scaleSprite = 0.25f;
    private int prime = 100;
    private float tachyDeadTimer = 0;

    private int randomVehicule;
    private String accelSoundRef = "F1SoundEffect-accel.mp3";
    private String decelSoundRef = "F1SoundEffect-ralenti.mp3";
    private String atlasRef = "postLoading_VehiculeGT.atlas";

    public VehiculeGT(MTCGame mtcGame, String player) {
        this.player = player;
        ...
        wheelTexture = textureAtlas.findRegion("GT_wheel");
        crashedWheelTexture = textureAtlas.findRegion("GT_crashedWheel");

        if(player.equals("ok")){
            this.vehiculeSoundAccel = mtcGame.getAssetManager().get("F1SoundEffect-accel.mp3", Music.class);
            this.vehiculeSoundDecel = mtcGame.getAssetManager().get("F1SoundEffect-ralenti.mp3", Music.class);

            TextureAtlas textureAtlasTabBord = mtcGame.getAssetManager().get("postLoading_VehiculeGT.atlas",TextureAtlas.class);
            jaugeCadran = new Image(textureAtlasTabBord.findRegion("formuleUn_jauge_cadran"));
            ...
            tableauDeBordTexture = new Image(textureAtlasTabBord.findRegion("tabBord-F1"));
            tableauNITROTexture = new Image(textureAtlasTabBord.findRegion("tabBord-F1-NITRO"));
            tachyCadran = new Image(textureAtlasTabBord.findRegion("formuleUn_tachy_cadran"));
        ...
        wheelShapeFloatArray = new float[]{-wheelTexture.getRegionWidth() / 8, wheelTexture.getRegionHeight() / 4,
                -wheelTexture.getRegionWidth() / 4, wheelTexture.getRegionHeight() / 8,
                -wheelTexture.getRegionWidth() / 4, -wheelTexture.getRegionHeight() / 8,
                -wheelTexture.getRegionWidth() / 8, -wheelTexture.getRegionHeight() / 4,
                wheelTexture.getRegionWidth() / 8, -wheelTexture.getRegionHeight() / 4,
                wheelTexture.getRegionWidth() / 4, -wheelTexture.getRegionHeight() / 8,
                wheelTexture.getRegionWidth() / 4, wheelTexture.getRegionHeight() / 8,
                wheelTexture.getRegionWidth() / 8, wheelTexture.getRegionHeight() / 4};

        chassisShapeFloatArray = new float[]{-carWidth / 2, -carHeight / 2, carWidth / 2,
                -carHeight / 2, carWidth / 2, carHeight/3, -carWidth / 2 * 0.8f, carHeight / 3};
    ...
    private TextureRegion createChassisTexture(){
        if(player.equals("ok")){
            chassisTexture = textureAtlas.findRegion("GT_chassis");
        }else{
            randomVehicule = MathUtils.random(0,3);
            if(randomVehicule == 0){
                chassisTexture = textureAtlas.findRegion("GT_chassis-2");
            }
            if(randomVehicule == 1){
                chassisTexture = textureAtlas.findRegion("GT_chassis-3");
            }
            if(randomVehicule == 2){
                chassisTexture = textureAtlas.findRegion("GT_chassis-4");
            }
            if(randomVehicule == 3){
                chassisTexture = textureAtlas.findRegion("GT_chassis-5");
            }
        }
    ...
    private TextureRegion createCrashedChassisTexture(){
        if(player.equals("ok")){
            crashedChassisTexture = textureAtlas.findRegion("GT_crashedChassis");
        }else{
            if(randomVehicule == 0){
                crashedChassisTexture = textureAtlas.findRegion("GT_crashedChassis-2");
            }
            if(randomVehicule == 1){
                crashedChassisTexture = textureAtlas.findRegion("GT_crashedChassis-3");
            }
            if(randomVehicule == 2){
                crashedChassisTexture = textureAtlas.findRegion("GT_crashedChassis-4");
            }
            if(randomVehicule == 3){
                crashedChassisTexture = textureAtlas.findRegion("GT_crashedChassis-5");
            }
        }
    ...
    private Vector2 createWheelLocalAnchorAPosition(){
        if(player.equals("ok")){
            wheelLocalAnchorAPosition  = new Vector2(-21.33f,-12.8f);
        }else{
            wheelLocalAnchorAPosition  = new Vector2(-21.33f,-6.8f);
        }
    ...
    private Group HUDTableauDeBordGroup(Stage stage, Viewport stageViewport, float vitesse, BitmapFont scoreFont) {
        ...
        jaugeGroup.addActor(jaugeFondBlanc);
        jaugeGroup.addActor(jaugeFondRouge);
        jaugeGroup.addActor(jaugeCadran);
        jaugeGroup.addActor(jaugeAiguille);
        stage.addActor(jaugeGroup);
