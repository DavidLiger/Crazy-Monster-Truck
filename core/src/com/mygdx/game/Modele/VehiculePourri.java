...
public class VehiculePourri implements VehiculeDef {
    ...
    //réglages
    private Vector2 jumpPower = new Vector2(80,100);
    ...
    private int chassisFixtureDefDensity = 1500;
    private int wheelFixtureDefFriction = 50;
    private float wheelFixtureDefRestitution = 0.001f;
    private float deadBoxShapeRadius = 9;
    private Vector2 neckLocalAnchorAPosition = new Vector2(-3,10);
    private Vector2 neckLocalAnchorBPosition = new Vector2(-2,-8.5f);
    private Vector2 wheelLocalAnchorAPosition;
    private float axisDefFrequencyHz = 3.6f;
    private float scaleSprite = 0.225f;
    private int prime = 50;
    private float tachyDeadTimer;

    private int randomVehicule;
    private String accelSoundRef = "PourrieSoundEffect-accel.mp3";
    private String decelSoundRef = "PourrieSoundEffect-decel.mp3";
    private String atlasRef = "postLoading_VehiculePourrie.atlas";

    public VehiculePourri(MTCGame mtcGame, String player){
        this.player = player;
        ...
        wheelTexture = textureAtlas.findRegion("pourrie_wheel");
        crashedWheelTexture = textureAtlas.findRegion("pourrie_crashedWheel");


        if(player.equals("ok")){
            this.vehiculeSoundAccel = mtcGame.getAssetManager().get("PourrieSoundEffect-accel.mp3",Music.class);
            this.vehiculeSoundDecel = mtcGame.getAssetManager().get("PourrieSoundEffect-decel.mp3",Music.class);

            TextureAtlas textureAtlasTabBord = mtcGame.getAssetManager().get("postLoading_VehiculePourrie.atlas",TextureAtlas.class);
            tableauDeBordTexture = new Image(textureAtlasTabBord.findRegion("tabBord-pourrie"));
            tableauNITROTexture = new Image(textureAtlasTabBord.findRegion("tabBord-pourrie-NITRO"));
            jaugeCadran = new Image(textureAtlasTabBord.findRegion("pourrie_jauge_cadran"));
            ...
            tachyCadran = new Image(textureAtlasTabBord.findRegion("pourrie_tachy_cadran"));
            tachyAiguille = new Image(textureAtlasTabBord.findRegion("tachy_aiguille"));
            tachyFondBlanc = new Image(textureAtlasTabBord.findRegion("tachy_pourrie_fondBlanc"));
            tachyFondRouge = new Image(textureAtlasTabBord.findRegion("tachy_fondRouge"));
        }

        wheelShapeFloatArray = new float[]{-wheelTexture.getRegionWidth()/5f, wheelTexture.getRegionHeight()/2.5f,
                -wheelTexture.getRegionWidth()/2.5f,wheelTexture.getRegionHeight()/5f,
                -wheelTexture.getRegionWidth()/2.5f,-wheelTexture.getRegionHeight()/5f,
                -wheelTexture.getRegionWidth()/5f,-wheelTexture.getRegionHeight()/2.5f,
                wheelTexture.getRegionWidth()/5f,-wheelTexture.getRegionHeight()/2.5f,
                wheelTexture.getRegionWidth()/2.5f,-wheelTexture.getRegionHeight()/5f,
                wheelTexture.getRegionWidth()/2.5f,wheelTexture.getRegionHeight()/5f,
                wheelTexture.getRegionWidth()/5f,wheelTexture.getRegionHeight()/2.5f};

        chassisShapeFloatArray = new float[]{-carWidth/2.35f, -carHeight/2.2f, carWidth/2.35f, -carHeight/2.2f,
                carWidth/7f , carHeight*0.6f, -carWidth/3.5f, carHeight*0.6f };
    }

    private TextureRegion createChassisTexture(){
        if(player.equals("ok")){
            chassisTexture = textureAtlas.findRegion("pourrie_chassis");
        }else{
            randomVehicule = MathUtils.random(0,3);
            if(randomVehicule == 0){
                chassisTexture = textureAtlas.findRegion("pourrie_chassis-2");
            }
            if(randomVehicule == 1){
                chassisTexture = textureAtlas.findRegion("pourrie_chassis-3");
            }
            if(randomVehicule == 2){
                chassisTexture = textureAtlas.findRegion("pourrie_chassis-4");
            }
            if(randomVehicule == 3){
                chassisTexture = textureAtlas.findRegion("pourrie_chassis-5");
            }
        }
    ...
    private TextureRegion createCrashedChassisTexture(){
        if(player.equals("ok")){
            crashedChassisTexture = textureAtlas.findRegion("pourrie_crashedChassis");
        }else{
            if(randomVehicule == 0){
                crashedChassisTexture = textureAtlas.findRegion("pourrie_crashedChassis-2");
            }
            if(randomVehicule == 1){
                crashedChassisTexture = textureAtlas.findRegion("pourrie_crashedChassis-3");
            }
            if(randomVehicule == 2){
                crashedChassisTexture = textureAtlas.findRegion("pourrie_crashedChassis-4");
            }
            if(randomVehicule == 3){
                crashedChassisTexture = textureAtlas.findRegion("pourrie_crashedChassis-5");
            }
        }
    ...
    private Vector2 createWheelLocalAnchorAPosition(){
        if(player.equals("ok")){
            wheelLocalAnchorAPosition  = new Vector2(-17.7f,-10.42f);
        }else{
            wheelLocalAnchorAPosition  = new Vector2(-17.7f,-5.8f);
        }
    ...
    public void setTachyAiguille(float vitesse) {
        tachyAiguille.setRotation(-vitesse * 4.68f);
    }
