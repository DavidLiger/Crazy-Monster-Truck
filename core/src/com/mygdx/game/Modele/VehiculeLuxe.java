...
public class VehiculeLuxe implements VehiculeDef{
    ...
    //réglages
    private float boostPower = 250;
    private Vector2 jumpPower = new Vector2(80, 120);
    ...
    private int chassisFixtureDefDensity = 1800;
    private int wheelFixtureDefFriction = 200;
    private float wheelFixtureDefRestitution = 0.001f;
    private float deadBoxShapeRadius = 10;
    private Vector2 neckLocalAnchorAPosition = new Vector2(-5,5f);
    private Vector2 neckLocalAnchorBPosition = new Vector2(-2,-8.5f);
    private Vector2 wheelLocalAnchorAPosition;
    private float axisDefFrequencyHz = 5f;
    private float scaleSprite = 0.25f;
    private int prime = 65;
    private float tachyDeadTimer = 0;

    private int randomVehicule;
    private String accelSoundRef = "LuxeSoundEffect-accel.mp3";
    private String decelSoundRef = "LuxeSoundEffect-decel.mp3";
    private String atlasRef = "postLoading_VehiculeLuxe.atlas";

    public VehiculeLuxe(MTCGame mtcGame, String player) {
        this.player = player;
        ...
        wheelTexture = textureAtlas.findRegion("luxe_wheel");
        crashedWheelTexture = textureAtlas.findRegion("luxe_crashedWheel");

        if(player.equals("ok")){
            this.vehiculeSoundAccel = mtcGame.getAssetManager().get("LuxeSoundEffect-accel.mp3", Music.class);
            this.vehiculeSoundDecel = mtcGame.getAssetManager().get("LuxeSoundEffect-decel.mp3", Music.class);

            TextureAtlas textureAtlasTabBord = mtcGame.getAssetManager().get("postLoading_VehiculeLuxe.atlas",TextureAtlas.class);
            jaugeCadran = new Image(textureAtlasTabBord.findRegion("Luxe_jauge_cadran"));
            ...
            tableauDeBordTexture = new Image(textureAtlasTabBord.findRegion("tabBord-Luxe"));
            tableauNITROTexture = new Image(textureAtlasTabBord.findRegion("tabBord-Luxe-NITRO"));
            tachyCadran = new Image(textureAtlasTabBord.findRegion("Luxe_tachy_cadran"));
...
//            digitalFont = mtcGame.getAssetManager().get("digitalTachy.fnt", BitmapFont.class);
        ...
        chassisShapeFloatArray = new float[]{-carWidth / 1.15f, -carHeight / 1.4f, carWidth / 1.45f,
                -carHeight / 1.4f, carWidth / 1.55f, carHeight *0.9f, -carWidth / 1.25f * 0.8f, carHeight *0.9f};
    ...
    private TextureRegion createChassisTexture(){
        if(player.equals("ok")){
            chassisTexture = textureAtlas.findRegion("luxe_chassis");
        }else{
            randomVehicule = MathUtils.random(0,3);
            if(randomVehicule == 0){
                chassisTexture = textureAtlas.findRegion("luxe_chassis-2");
            }
            if(randomVehicule == 1){
                chassisTexture = textureAtlas.findRegion("luxe_chassis-3");
            }
            if(randomVehicule == 2){
                chassisTexture = textureAtlas.findRegion("luxe_chassis-4");
            }
            if(randomVehicule == 3){
                chassisTexture = textureAtlas.findRegion("luxe_chassis-5");
            }
        }
    ...
    private TextureRegion createCrashedChassisTexture(){
        if(player.equals("ok")){
            crashedChassisTexture = textureAtlas.findRegion("luxe_crashedChassis");
        }else{
            if(randomVehicule == 0){
                crashedChassisTexture = textureAtlas.findRegion("luxe_crashedChassis-2");
            }
            if(randomVehicule == 1){
                crashedChassisTexture = textureAtlas.findRegion("luxe_crashedChassis-3");
            }
            if(randomVehicule == 2){
                crashedChassisTexture = textureAtlas.findRegion("luxe_crashedChassis-4");
            }
            if(randomVehicule == 3){
                crashedChassisTexture = textureAtlas.findRegion("luxe_crashedChassis-5");
            }
        }
