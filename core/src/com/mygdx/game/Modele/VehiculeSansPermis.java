...
public class VehiculeSansPermis implements VehiculeDef{
    ...
    private int chassisFixtureDefDensity = 800;
    private int wheelFixtureDefFriction = 60;
    private float wheelFixtureDefRestitution = 0.001f;
    private float deadBoxShapeRadius = 10;
    private Vector2 neckLocalAnchorAPosition = new Vector2(-3,11.5f);
    private Vector2 neckLocalAnchorBPosition = new Vector2(-2,-8.5f);
    private Vector2 wheelLocalAnchorAPosition;
    private float axisDefFrequencyHz = 3.7f;
    private float scaleSprite = 0.25f;
    private int prime = 25;
    private float tachyDeadTimer;

    private int randomVehicule;
    private String accelSoundRef = "SansPermisSoundEffect-accel.mp3";
    private String decelSoundRef = "SansPermisSoundEffect-decel.mp3";
    private String atlasRef = "postLoading_VehiculeSansPermis.atlas";

    public VehiculeSansPermis(MTCGame mtcGame, String player) {
        this.player = player;
        ...
        wheelTexture = textureAtlas.findRegion("sansPermis_wheel");
        crashedWheelTexture = textureAtlas.findRegion("sansPermis_crashedWheel");

        if(player.equals("ok")){
            this.vehiculeSoundAccel = mtcGame.getAssetManager().get("SansPermisSoundEffect-accel.mp3", Music.class);
            this.vehiculeSoundDecel = mtcGame.getAssetManager().get("SansPermisSoundEffect-decel.mp3", Music.class);

            TextureAtlas textureAtlasTabBord = mtcGame.getAssetManager().get("postLoading_VehiculeSansPermis.atlas",TextureAtlas.class);
            jaugeCadran = new Image(textureAtlasTabBord.findRegion("sansPermis_jauge_cadran"));
            ...
            tableauDeBordTexture = new Image(textureAtlasTabBord.findRegion("tabBord-sansPermis"));
            tableauNITROTexture = new Image(textureAtlasTabBord.findRegion("tabBord-sansPermis-NITRO"));
            tachyCadran = new Image(textureAtlasTabBord.findRegion("sansPermis_tachy_cadran"));
        ...
        chassisShapeFloatArray = new float[]{-carWidth / 2.5f, -carHeight / 1.5f, carWidth / 2.05f,
                -carHeight / 1.5f, carWidth / 6.9f, carHeight *0.65f, -carWidth / 4.5f, carHeight *0.65f};
    ...
    private TextureRegion createChassisTexture(){
        if(player.equals("ok")){
            chassisTexture = textureAtlas.findRegion("sansPermis_chassis");
        }else{
            randomVehicule = MathUtils.random(0,3);
            if(randomVehicule == 0){
                chassisTexture = textureAtlas.findRegion("sansPermis_chassis-2");
            }
            if(randomVehicule == 1){
                chassisTexture = textureAtlas.findRegion("sansPermis_chassis-3");
            }
            if(randomVehicule == 2){
                chassisTexture = textureAtlas.findRegion("sansPermis_chassis-4");
            }
            if(randomVehicule == 3){
                chassisTexture = textureAtlas.findRegion("sansPermis_chassis-5");
            }
        }
    ...
    private TextureRegion createCrashedChassisTexture(){
        if(player.equals("ok")){
            crashedChassisTexture = textureAtlas.findRegion("sansPermis_crashedChassis");
        }else{
            if(randomVehicule == 0){
                crashedChassisTexture = textureAtlas.findRegion("sansPermis_crashedChassis-2");
            }
            if(randomVehicule == 1){
                crashedChassisTexture = textureAtlas.findRegion("sansPermis_crashedChassis-3");
            }
            if(randomVehicule == 2){
                crashedChassisTexture = textureAtlas.findRegion("sansPermis_crashedChassis-4");
            }
            if(randomVehicule == 3){
                crashedChassisTexture = textureAtlas.findRegion("sansPermis_crashedChassis-5");
            }
        }
    ...
    private Vector2 createWheelLocalAnchorAPosition(){
        if(player.equals("ok")){
            wheelLocalAnchorAPosition  = new Vector2(-19.33f,-14.8f);
        }else{
            wheelLocalAnchorAPosition  = new Vector2(-16.33f,-8.8f);
        }
