...
public class VehiculeLegende implements VehiculeDef{
    ...
    private Vector2 jumpPower = new Vector2(125, 125);
    ...
    private int chassisFixtureDefDensity = 1000;
    private int wheelFixtureDefFriction = 120;
    private float wheelFixtureDefRestitution = 0.001f;
    private float deadBoxShapeRadius = 10;
    private Vector2 neckLocalAnchorAPosition = new Vector2(-0.8f,3.5f);
    private Vector2 neckLocalAnchorBPosition = new Vector2(-2,-8.5f);
    private Vector2 wheelLocalAnchorAPosition;
    private float axisDefFrequencyHz = 6;
    private float scaleSprite = 0.25f;
    private int prime = 85;
    private float tachyDeadTimer = 0;

    private int randomVehicule;
    private String accelSoundRef = "LegendeSoundEffect-accel.mp3";
    private String decelSoundRef = "LegendeSoundEffect-decel.mp3";
    private String atlasRef = "postLoading_VehiculeLegende.atlas";

    public VehiculeLegende(MTCGame mtcGame, String player) {
        this.player = player;
        ...
        wheelTexture = textureAtlas.findRegion("legende_wheel");
        crashedWheelTexture = textureAtlas.findRegion("legende_crashedWheel");

        if(player.equals("ok")){
            this.vehiculeSoundAccel = mtcGame.getAssetManager().get("LegendeSoundEffect-accel.mp3", Music.class);
            this.vehiculeSoundDecel = mtcGame.getAssetManager().get("LegendeSoundEffect-decel.mp3", Music.class);

            TextureAtlas textureAtlasTabBord = mtcGame.getAssetManager().get("postLoading_VehiculeLegende.atlas",TextureAtlas.class);
            jaugeCadran = new Image(textureAtlasTabBord.findRegion("Legende_jauge_cadran"));
            ...
            tableauDeBordTexture = new Image(textureAtlasTabBord.findRegion("tabBord-Legende"));
            tableauNITROTexture = new Image(textureAtlasTabBord.findRegion("tabBord-Legende-NITRO"));
            tachyCadran = new Image(textureAtlasTabBord.findRegion("Legende_tachy_cadran"));
        ...
        chassisShapeFloatArray = new float[]{-carWidth / 2.2f, -carHeight / 1.5f, carWidth / 2.05f,
                -carHeight / 1.5f, carWidth / 6.9f, carHeight *0.55f, -carWidth / 4.5f, carHeight *0.55f};
    ...
    private TextureRegion createChassisTexture(){
        if(player.equals("ok")){
            chassisTexture = textureAtlas.findRegion("legende_chassis");
        }else{
            randomVehicule = MathUtils.random(0,3);
            if(randomVehicule == 0){
                chassisTexture = textureAtlas.findRegion("legende_chassis-2");
            }
            if(randomVehicule == 1){
                chassisTexture = textureAtlas.findRegion("legende_chassis-3");
            }
            if(randomVehicule == 2){
                chassisTexture = textureAtlas.findRegion("legende_chassis-4");
            }
            if(randomVehicule == 3){
                chassisTexture = textureAtlas.findRegion("legende_chassis-5");
            }
        }
    ...
    private TextureRegion createCrashedChassisTexture(){
        if(player.equals("ok")){
            crashedChassisTexture = textureAtlas.findRegion("legende_crashedChassis");
        }else{
            if(randomVehicule == 0){
                crashedChassisTexture = textureAtlas.findRegion("legende_crashedChassis-2");
            }
            if(randomVehicule == 1){
                crashedChassisTexture = textureAtlas.findRegion("legende_crashedChassis-3");
            }
            if(randomVehicule == 2){
                crashedChassisTexture = textureAtlas.findRegion("legende_crashedChassis-4");
            }
            if(randomVehicule == 3){
                crashedChassisTexture = textureAtlas.findRegion("legende_crashedChassis-5");
            }
        }
    ...
    private Vector2 createWheelLocalAnchorAPosition(){
        if(player.equals("ok")){
            ...
            wheelLocalAnchorAPosition  = new Vector2(-22.33f,-12.8f);
        }
