...
public class VehiculeBerline implements VehiculeDef{
    ...
    private Vector2 jumpPower = new Vector2(60, 100);
    ...
    private Vector2 neckLocalAnchorAPosition = new Vector2(-3.5f,2.5f);
    private Vector2 neckLocalAnchorBPosition = new Vector2(-2,-8.5f);
    private Vector2 wheelLocalAnchorAPosition;
    private float axisDefFrequencyHz = 4;
    private float scaleSprite = 0.25f;
    private int prime = 40;
    private float tachyDeadTimer = 0;

    private int randomVehicule;
    private String accelSoundRef = "BerlineSoundEffect-accel.mp3";
    private String decelSoundRef = "BerlineSoundEffect-decel.mp3";
    private String atlasRef = "postLoading_VehiculeBerline.atlas";


    public VehiculeBerline(MTCGame mtcGame, String player) {
        this.player = player;
        ...
        wheelTexture = textureAtlas.findRegion("berline_wheel");
        crashedWheelTexture = textureAtlas.findRegion("berline_crashedWheel");

        if(player.equals("ok")){
            this.vehiculeSoundAccel = mtcGame.getAssetManager().get("BerlineSoundEffect-accel.mp3", Music.class);
            this.vehiculeSoundDecel = mtcGame.getAssetManager().get("BerlineSoundEffect-decel.mp3", Music.class);

            TextureAtlas textureAtlasTabBord = mtcGame.getAssetManager().get("postLoading_VehiculeBerline.atlas",TextureAtlas.class);
            jaugeCadran = new Image(textureAtlasTabBord.findRegion("Berline_jauge_cadran"));
            ...
            tableauDeBordTexture = new Image(textureAtlasTabBord.findRegion("tabBord-Berline"));
            tableauNITROTexture = new Image(textureAtlasTabBord.findRegion("tabBord-Berline-NITRO"));
            tachyCadran = new Image(textureAtlasTabBord.findRegion("Berline_tachy_cadran"));
        ...
        chassisShapeFloatArray = new float[]{-carWidth / 1.95f, -carHeight / 1.5f, carWidth / 2.05f,
                -carHeight / 1.5f, carWidth / 6.9f, carHeight *0.65f, -carWidth / 2.6f, carHeight *0.65f};
    ...
    private TextureRegion createChassisTexture(){
        if(player.equals("ok")){
            chassisTexture = textureAtlas.findRegion("berline_chassis");
        }else{
            randomVehicule = MathUtils.random(0,3);
            if(randomVehicule == 0){
                chassisTexture = textureAtlas.findRegion("berline_chassis-2");
            }
            if(randomVehicule == 1){
                chassisTexture = textureAtlas.findRegion("berline_chassis-3");
            }
            if(randomVehicule == 2){
                chassisTexture = textureAtlas.findRegion("berline_chassis-4");
            }
            if(randomVehicule == 3){
                chassisTexture = textureAtlas.findRegion("berline_chassis-5");
            }
        }
    ...
    private TextureRegion createCrashedChassisTexture(){
        if(player.equals("ok")){
            crashedChassisTexture = textureAtlas.findRegion("berline_crashedChassis");
        }else{
            if(randomVehicule == 0){
                crashedChassisTexture = textureAtlas.findRegion("berline_crashedChassis-2");
            }
            if(randomVehicule == 1){
                crashedChassisTexture = textureAtlas.findRegion("berline_crashedChassis-3");
            }
            if(randomVehicule == 2){
                crashedChassisTexture = textureAtlas.findRegion("berline_crashedChassis-4");
            }
            if(randomVehicule == 3){
                crashedChassisTexture = textureAtlas.findRegion("berline_crashedChassis-5");
            }
        }
    ...
    private Vector2 createWheelLocalAnchorAPosition(){
        if(player.equals("ok")){
            ...
            wheelLocalAnchorAPosition  = new Vector2(-20.33f,-8.8f);
        }
