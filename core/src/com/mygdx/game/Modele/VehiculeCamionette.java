...
public class VehiculeCamionette implements VehiculeDef{
    ...
    //réglages
    private float boostPower = 120;
    private Vector2 jumpPower = new Vector2(25, 75);
    ...
    private int chassisFixtureDefDensity = 2500;
    private int wheelFixtureDefFriction = 150;
    private float wheelFixtureDefRestitution = 0.001f;
    private float deadBoxShapeRadius = 10;
    private Vector2 neckLocalAnchorAPosition = new Vector2(14,12.5f);
    private Vector2 neckLocalAnchorBPosition = new Vector2(-2,-8.5f);
    private Vector2 wheelLocalAnchorAPosition;
    private float axisDefFrequencyHz = 2.3f;
    private float scaleSprite = 0.25f;
    private int prime = 115;
    private float tachyDeadTimer = 0;

    private int randomVehicule;
    private String accelSoundRef = "CamionetteSoundEffect-accel.mp3";
    private String decelSoundRef = "CamionetteSoundEffect-decel.mp3";
    private String atlasRef = "postLoading_VehiculeCamionette.atlas";


    public VehiculeCamionette(MTCGame mtcGame, String player) {
        this.player = player;
        ...
        wheelTexture = textureAtlas.findRegion("camionette_wheel");
        crashedWheelTexture = textureAtlas.findRegion("camionette_crashedWheel");

        if(player.equals("ok")){
            this.vehiculeSoundAccel = mtcGame.getAssetManager().get("CamionetteSoundEffect-accel.mp3", Music.class);
            this.vehiculeSoundDecel = mtcGame.getAssetManager().get("CamionetteSoundEffect-decel.mp3", Music.class);

            TextureAtlas textureAtlasTabBord = mtcGame.getAssetManager().get("postLoading_VehiculeCamionette.atlas",TextureAtlas.class);
            jaugeCadran = new Image(textureAtlasTabBord.findRegion("Camionette_jauge_cadran"));
            ...
            tableauDeBordTexture = new Image(textureAtlasTabBord.findRegion("tabBord-Camionette"));
            tableauNITROTexture = new Image(textureAtlasTabBord.findRegion("tabBord-Camionette-NITRO"));
            tachyCadran = new Image(textureAtlasTabBord.findRegion("Camionette_tachy_cadran"));
...
//            digitalFont = mtcGame.getAssetManager().get("digitalTachy.fnt", BitmapFont.class);
        ...
        chassisShapeFloatArray = new float[]{-carWidth / 1.9f, -carHeight / 1.35f, carWidth / 1.85f,
                -carHeight / 1.35f, carWidth / 2.4f, carHeight *0.8f, -carWidth / 2.05f, carHeight *0.8f};
    ...
    private TextureRegion createChassisTexture(){
        if(player.equals("ok")){
            chassisTexture = textureAtlas.findRegion("camionette_chassis");
        }else{
            randomVehicule = MathUtils.random(0,3);
            if(randomVehicule == 0){
                chassisTexture = textureAtlas.findRegion("camionette_chassis-2");
            }
            if(randomVehicule == 1){
                chassisTexture = textureAtlas.findRegion("camionette_chassis-3");
            }
            if(randomVehicule == 2){
                chassisTexture = textureAtlas.findRegion("camionette_chassis-4");
            }
            if(randomVehicule == 3){
                chassisTexture = textureAtlas.findRegion("camionette_chassis-5");
            }
        }
    ...
    private TextureRegion createCrashedChassisTexture(){
        if(player.equals("ok")){
            crashedChassisTexture = textureAtlas.findRegion("camionette_crashedChassis");
        }else{
            if(randomVehicule == 0){
                crashedChassisTexture = textureAtlas.findRegion("camionette_crashedChassis-2");
            }
            if(randomVehicule == 1){
                crashedChassisTexture = textureAtlas.findRegion("camionette_crashedChassis-3");
            }
            if(randomVehicule == 2){
                crashedChassisTexture = textureAtlas.findRegion("camionette_crashedChassis-4");
            }
            if(randomVehicule == 3){
                crashedChassisTexture = textureAtlas.findRegion("camionette_crashedChassis-5");
            }
        }
    ...
    private Vector2 createWheelLocalAnchorAPosition(){
        if(player.equals("ok")){
            wheelLocalAnchorAPosition  = new Vector2(-20,-18.8f);
        }else{
            wheelLocalAnchorAPosition  = new Vector2(-20,-9.8f);
        }
