...
public class VehiculeChantier implements VehiculeDef{
    ...
    //réglages
    private float boostPower = 50;
    private Vector2 jumpPower = new Vector2(0, 50);
    ...
    private int chassisFixtureDefDensity = 5000;
    private int wheelFixtureDefFriction = 600;
    private float wheelFixtureDefRestitution = 0.001f;
    private float deadBoxShapeRadius = 10;
    private Vector2 neckLocalAnchorAPosition = new Vector2(27,17.5f);
    private Vector2 neckLocalAnchorBPosition = new Vector2(-2,-8.5f);
    private Vector2 wheelLocalAnchorAPosition;
    private float axisDefFrequencyHz = 2;
    private float scaleSprite = 0.25f;
    private int prime = 150;
    private float tachyDeadTimer = 0;

    private int randomVehicule;
    private String accelSoundRef = "ChantierSoundEffect-accel.mp3";
    private String decelSoundRef = "ChantierSoundEffect-decel.mp3";
    private String atlasRef = "postLoading_VehiculeChantier.atlas";

    public VehiculeChantier(MTCGame mtcGame, String player) {
        this.player = player;
        ...
        wheelTexture = textureAtlas.findRegion("chantier_wheel");
        crashedWheelTexture = textureAtlas.findRegion("chantier_crashedWheel");

        if(player.equals("ok")){
            this.vehiculeSoundAccel = mtcGame.getAssetManager().get("ChantierSoundEffect-accel.mp3", Music.class);
            this.vehiculeSoundDecel = mtcGame.getAssetManager().get("ChantierSoundEffect-decel.mp3", Music.class);

            TextureAtlas textureAtlasTabBord = mtcGame.getAssetManager().get("postLoading_VehiculeChantier.atlas",TextureAtlas.class);
            jaugeCadran = new Image(textureAtlasTabBord.findRegion("Chantier_jauge_cadran"));
            ...
            tableauDeBordTexture = new Image(textureAtlasTabBord.findRegion("tabBord-Chantier"));
            tableauNITROTexture = new Image(textureAtlasTabBord.findRegion("tabBord-Chantier-NITRO"));
            tachyCadran = new Image(textureAtlasTabBord.findRegion("Chantier_tachy_cadran"));
...
//            digitalFont = mtcGame.getAssetManager().get("digitalTachy.fnt", BitmapFont.class);
        ...
        chassisShapeFloatArray = new float[]{-carWidth / 1.3f, -carHeight / 0.8f, carWidth / 1.45f,
                -carHeight / 0.8f, carWidth / 3f, carHeight *0.5f, -carWidth / 1.4f , carHeight *0.35f};
    }
    ...
    private TextureRegion createChassisTexture(){
        if(player.equals("ok")){
            chassisTexture = textureAtlas.findRegion("chantier_chassis");
        }else{
            randomVehicule = MathUtils.random(0,3);
            if(randomVehicule == 0){
                chassisTexture = textureAtlas.findRegion("chantier_chassis-2");
            }
            if(randomVehicule == 1){
                chassisTexture = textureAtlas.findRegion("chantier_chassis-3");
            }
            if(randomVehicule == 2){
                chassisTexture = textureAtlas.findRegion("chantier_chassis-4");
            }
            if(randomVehicule == 3){
                chassisTexture = textureAtlas.findRegion("chantier_chassis-5");
            }
        }
    ...
    private TextureRegion createCrashedChassisTexture(){
        if(player.equals("ok")){
            crashedChassisTexture = textureAtlas.findRegion("chantier_crashedChassis");
        }else{
            if(randomVehicule == 0){
                crashedChassisTexture = textureAtlas.findRegion("chantier_crashedChassis-2");
            }
            if(randomVehicule == 1){
                crashedChassisTexture = textureAtlas.findRegion("chantier_crashedChassis-3");
            }
            if(randomVehicule == 2){
                crashedChassisTexture = textureAtlas.findRegion("chantier_crashedChassis-4");
            }
            if(randomVehicule == 3){
                crashedChassisTexture = textureAtlas.findRegion("chantier_crashedChassis-5");
            }
        }
    ...
    private Vector2 createWheelLocalAnchorAPosition(){
        if(player.equals("ok")){
            wheelLocalAnchorAPosition  = new Vector2(-34.33f,-18.8f);
        }else{
            wheelLocalAnchorAPosition  = new Vector2(-34.33f,-12.8f);
        }
